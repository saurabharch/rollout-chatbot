...
class HelpCenter {
  ...
  constructor(options) {
    this.SERVER_URL = options.SERVER_URL; // https://rollout-cms-server-prod.herokuapp.com
  ...
  myrequest(options, callback, log) {
    ...
    .then(function (res) {
      ...
      if (res && res.status == 200 && res.data) {
        ...
        if (callback) {
          callback(RolloutClient.getErr({message: "Response status not 200"}, options, res), null, null);
        }
      }
    })
