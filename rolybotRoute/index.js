...
// DEV
// const { MessagePipeline } = require('./rolloutChatbotPlugs/MessagePipeline');
// const { DirectivesChatbotPlug } = require('./rolloutChatbotPlugs/DirectivesChatbotPlug');
// const { SplitsChatbotPlug } = require('./rolloutChatbotPlugs/SplitsChatbotPlug');
// const { MarkbotChatbotPlug } = require('./rolloutChatbotPlugs/MarkbotChatbotPlug');
// const { WebhookChatbotPlug } = require('./rolloutChatbotPlugs/WebhookChatbotPlug');


// PROD
const { MessagePipeline } =  require('@saurabharch/rollout-chatbot-plugs/MessagePipeline');
const { DirectivesChatbotPlug } = require('@saurabharch/rollout-chatbot-plugs/DirectivesChatbotPlug');
const { SplitsChatbotPlug } = require('@saurabharch/rollout-chatbot-plugs/SplitsChatbotPlug');
const { MarkbotChatbotPlug } = require('@saurabharch/rollout-chatbot-plugs/MarkbotChatbotPlug');
const { WebhookChatbotPlug } = require('@saurabharch/rollout-chatbot-plugs/WebhookChatbotPlug');
let path = require("path");
let fs = require('fs');

const { RolloutChatbotClient } = require('@saurabharch/rollout-chatbot-client');
...
async function execFaq(req, res, faqs, botId, message, token, bot) {
  ...
  const tdclient = new RolloutChatbotClient(
  {
    request: req,
    APIKEY: '__APIKEY__',
    APIURL: APIURL
...
function apiurl() {
  const server = "pre";
  //const server = "prod";
  const API_URL_PRE = 'https://rollout-server-pre.herokuapp.com';
  const API_URL_PROD = 'https://api.rollout.com/v2';
  // choose a server
...
router.get('/', (req, res) => {
  res.send('Hello Rolybot!');
});

function startApp(settings, completionCallback) {
  console.log("Starting Rolybot with Settings:", settings);
  ...
  if (!settings.API_ENDPOINT) {
    ...
    console.log("(Rolybot) settings.API_ENDPOINT:", APIURL);
  }
  if (!settings.log) {
    log = false;
  }
  ...
  console.log("Starting Rollbot connector...");
  console.log("(Rollbot) Connecting to mongodb...");
