...
console.debug("webhook_origin: "+webhook_origin);
...
class FaqBotSupport {
    ...
    getMessage(key, lang, labelsObject) {
        ...
        console.debug('getMessage: ' + key + ' ' + lang+ ' ' + JSON.stringify(labelsObject) );
...
// usa api di sponziello parseReply: https://github.com/saurabharch/rollout-nodejs-libs/blob/master/rollout-chatbot-util/index.js 

    parseMicrolanguage(text, message, bot, faq, disableWebHook, json) { 
        var that = this;
        return new Promise(async (resolve, reject) => {
            console.debug('parseMicrolanguage message: ' + JSON.stringify(message) );
            var reply = RolloutChatbotUtil.parseReply(text);
            console.debug('parseReply: ' + JSON.stringify(reply) );
            ...
            // console.debug('message.language: '+ message.language );
            // if (message.language) {
            ...
            if (disableWebHook === false && bot.webhook_enabled ===true && (faq.webhook_enabled  === true || reply.webhook)) {
                ...
                if (!webhookurl) {
                    console.debug("webhookurl is undefined return standard");
                    return resolve(messageReply);
                }
                    
                var token = "";// req.body.token (da ext)             
                ...
                return request({                        
                    ...
                    // }).then(response => {
                        ...
                        if (err) {
                            console.error("Error from webhook reply of getParsedMessage. Return standard reply", err);
                         ...
                         if (response.statusCode >= 400) {      
                            console.verbose("The ChatBot webhook return error http status code. Return standard reply", response);            
                            return resolve(messageReply);
                        }

                        if (!json) { //the webhook return empty body
                            console.verbose("The ChatBot webhook return no json. Return standard reply", response);
                            return resolve(messageReply);
                        }
                       
                        console.debug("webhookurl repl_message ", response);

                        var text = undefined;
                        if(json && json.text===undefined) {
                            console.verbose("webhookurl json is defined but text not. return standard reply",{json:json, response:response});
                            // text = 'Field text is not defined in the webhook respose of the faq with id: '+ faq._id+ ". Error: " + JSON.stringify(response);
                            return resolve(messageReply);
                        }else {
                            text = json.text;
                        }
                        console.debug("webhookurl text:  "+ text);

                        // // let cloned_message = Object.assign({}, messageReply);
                        // let cloned_message =  message;
                        // console.debug("cloned_message :  ",cloned_message);

                        // if (json.attributes) {
                        //     if (!cloned_message.attributes) {
                        //         cloned_message.attributes = {}
                        //     }
                        //     console.debug("ChatBot webhook json.attributes: ",json.attributes);
                        //     for(const [key, value] of Object.entries(json.attributes)) {
                        //         cloned_message.attributes[key] = value
                        //     }
                        // }

                        // console.debug("cloned_message after attributes:  ",cloned_message);
    ...
    //         console.debug("getParsedMessage ******",text);
    //         var repl_message = {};
    ...
    //             console.debug("frameurl ", frameurl)
    ...
    //                         console.error("Error from webhook reply of getParsedMessage", err);
    //                         return resolve(bot_answer);
    //                     }
    //                     // if (response.statusCode >= 400) {                  
    ...
    //         }else {
    //             console.debug("repl_message ", repl_message)
    ...
    getBotMessage(botAnswer, projectid, bot, message, threshold) {
        var that = this;
          return new Promise(function(resolve, reject) {
              ...
              console.debug('botAnswer', botAnswer);
                // var found = false;
                var bot_answer={};
                        ...
                        .lean().             //fai cache  
                         exec(function (err, faqs) {
                           ...
                           if (faqs && faqs.length>0) {
                                console.debug("faqs exact", faqs);  

                                bot_answer.text=faqs[0].answer;   
                                
                                console.debug("bot_answer exact", bot_answer);  
                                // found = true;
                                // return resolve(bot_answer);

                                if (message.channel.name == "rollout") {    //why this contition on rollout channel? bacause only rollout support parsed replies?
                                    console.debug("faqBotSupport message.channel.name is rollout",message);
                                    ...
                                    console.debug("faqBotSupport message.channel.name is not rollout returning default",message);
                                    return resolve(bot_answer);
                                }
